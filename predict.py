...
def main():
    ...
    model = network.modeling.__dict__[opts.model](num_classes=19, BB= 'RN50')
    ...
    with torch.no_grad():        
        for i in range(len(img_list)):
            ...
            out,feat = model(img)
            pred = out.detach().max(1)[1].cpu().numpy()[0] # HW
            colorized_preds = decode_fn(pred).astype('uint8')
            colorized_preds = Image.fromarray(colorized_preds)
            if opts.save_val_results_to:
                print(img_list[i].split('/')[-1])
                colorized_preds.save(os.path.join(opts.save_val_results_to,img_list[i].split('/')[-1]))            
